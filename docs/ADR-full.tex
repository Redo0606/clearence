...
\begin{enumerate}
    ...
    \item \textbf{Theory-grounded pipeline} (\texttt{ontology\_builder/}): Full 6-step pipeline with sequential extraction, taxonomy building, graph merge, OWL~2~RL reasoning, and ontology-grounded RAG.
\end{enumerate}
...
├── ontology/                 # Schema, canonicalizer, rules
...
\begin{enumerate}
    ...
    \item \textbf{Chunk} --- \texttt{chunk\_text()} with configurable overlap
    \item \textbf{Extract} --- 3-stage sequential: classes $\rightarrow$ instances $\rightarrow$ relations + axioms (or legacy single-shot)
    \item \textbf{Merge} --- \texttt{update\_graph()} with canonicalization
    \item \textbf{Inference} --- LLM relation inference (optional)
    \item \textbf{Reasoning} --- OWL~2~RL fixpoint (optional)
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Persistence}: JSON in \texttt{documents/ontology\_graphs/} with \texttt{.meta.json} sidecar (name, description, documents, stats)
    \item \textbf{Activation}: Load JSON into graph\_store; rebuild QA index
\end{itemize}
...
Document (PDF/DOCX/TXT/MD)
  ...
  -> ontology_builder.update_graph() [canonicalize, merge]
  -> relation_inferer.infer_relations() [optional]
  -> reasoning.engine.run_inference() [optional]
  -> OntologyGraph
  -> graph_store.set_graph()
  -> qa.graph_index.build_index()
...
\texttt{CHUNK\_SIZE} / \texttt{CHUNK\_OVERLAP} & 1200 / 200 & Chunking \\
